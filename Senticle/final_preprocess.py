...
if len(df[df.index <= datetime.datetime(year=df.index[0].year, month=df.index[0].month, day=df.index[0].day, hour=15,
                                        minute=30)].index) == 0:
    pass
else:
    df = df.drop(df[df.index <= datetime.datetime(year=df.index[0].year, month=df.index[0].month, day=df.index[0].day,
                                                  hour=15, minute=30)].index, axis=0)

# 학습데이터의 마지막 3:30 이후 데이터는 버리기
if len(df[df.index >= datetime.datetime(year=df.index[-1].year, month=df.index[-1].month, day=df.index[-1].day, hour=15,
                                        minute=30)].index) == 0:
    pass
else:
